...
# pdb_name is assumed to have CHARMM pdb format, not RCSB format.
# MISC
mode_id = 10
N_FRAME = 20
N_AMP   = 2
...
with open(pdb_name+'.pdb','r') as pdb_file:
	...
	for each_line in pdb_file:
		if each_line.startswith('ATOM'):
			atom_count += 1
			# This is more cumbersome than np.genfromtxt then slicing but:
			# 1) It is error-proof with large beta values.
			# 2) Many programs have strict formatting requiremens for input PDB files.
