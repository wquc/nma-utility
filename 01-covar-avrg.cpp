/*************************************************************
o Calculate covariance using the following equation
  C_{ij}=\frac{\sum_M(\vec{a}_{iM}\cdot\vec{a}_{jM})/\lambda_M}
       ...
       [\sum_M(\vec{a}_{jM}\cdot\vec{a}_{jM})/\lambda_M]}}
                                                          Q.W.
o Example of input file: [input-cutoff10.conf]
#----------------------------------------------------
...
o Format of eigenvector file
  [mode_id] [mode_eigenvalue] : [x1] [y1] [z1] ... [xn] [yn] [zn]

o Compile: g++ -std=c++11 cov-avrg.cpp

o Usage: ./a.out input-cutoff10.conf
***************************************************************/
...
void check_config (const Config& config) {
    ...
    for (auto imode: config.mode_list) {
...
bool calc_eigenvec_corr (const std::string& out_name, const unsigned int& vecdim) {
    ...
    for(unsigned int i=0; i<vecdim; i++) {
        ...
        std::cout << std::setw(5) << std::fixed << std::setprecision(2) 
                  << static_cast<double>(i+1)/vecdim*100 << "% completed\r";
        for(unsigned int j=0; j<vecdim; j++){
            cij = sqrt(product_ii[i]*product_ii[j]);
            cij = product_ij[i*vecdim+j]/cij;
            out_file << std::setw(6) << i+1 
                     << std::setw(6) << j+1 
                     << std::fixed << std::setw(12) << std::setprecision(6) << cij 
                     << std::endl;
        }
    }
    out_file.close();
