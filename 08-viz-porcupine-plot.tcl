#-------------------------------------------------------------------
# o Draws Nuclear Magnetic Moment vectors on all atoms indicated by 
#   the *.nmm file.
# o The *.nmm file must be associated with a proper *.pdb file.
# o Description:
#   After loading a *.pdb file, start this program and when prompted
#   read in the associate *.nmm file, select the molecule id and the 
#   scaling factor. The indicated vectors will be displayed using "draw".
# o Author:
...
proc nmm {} {
    ...
    foreach line [split [read $data] \n] {
        if {[lindex $line 0] == "ATOM"} {
            ...
            set coords2 [vecscale 0.0 $coords3]
            set coords2 [vecadd $coords1 $coords2]
            set coords3 [vecadd $coords1 $coords3]
    
            draw cylinder $coords1 $coords2 radius 0.0
